...
WHERE (PassedCourses.course = RecommendedBranch.course AND StudentBranches.program = RecommendedBranch.program 
...
--(PassedCourses.course IN (SELECT course FROM MandatoryBranch) OR PassedCourses.course IN (SELECT course FROM MandatoryProgram))

AND

PassedCourses.student IN(SELECT student FROM RecommendedCoursesHelper WHERE sum >=10)
